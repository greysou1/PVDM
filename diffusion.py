...
def save_image(xs, tgt_pose, src, t, save_folder):
    samples = xs[-1].cuda()
    ...
    image = Image.fromarray(fake_imgs[0])
    draw = ImageDraw.Draw(image)
    font = ImageFont.truetype("OpenSans-VariableFont_wdth,wght.ttf", 20)

    draw.text((5, 5), f"timestep = {t}, T=1000", fill ="red", font = font, align ="left") 
    image.save(f'{save_folder}/at_t{t}.png')
...
def ddim_steps(x, seq, model, b, x_cond, diffusion = None, **kwargs):
    ...
    with torch.no_grad():
        ...
        for i, j in tqdm.tqdm(zip(reversed(seq), reversed(seq_next))):
            ...
            # save_image([xt], tgt_pose, src, i, kwargs["save_folder"])
...
class GaussianDiffusion:
    ...
    def p_sample_loop_progressive(
        ...
        if progress:
            ...
            indices = tqdm(indices, total=len(indices))
